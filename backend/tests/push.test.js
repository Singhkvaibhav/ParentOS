...
describe("registering a push token", () => {
  test("rejects a malformed token", async () => {
    ...
    const res = await user.agent.post("/api/notifications/push-token").send({ token: "not-a-real-token", platform: "ios" });
  ...
  test("rejects an invalid platform", async () => {
    ...
    const res = await user.agent.post("/api/notifications/push-token").send({ token: VALID_TOKEN, platform: "windows-phone" });
    expect(res.status).toBe(400);
  ...
  test("accepts a well-formed token and records it against the caller", async () => {
    ...
    const res = await user.agent.post("/api/notifications/push-token").send({ token: VALID_TOKEN, platform: "ios" });
  ...
  test("re-registering the same token to a different user reassigns it, not duplicates it", async () => {
    ...
    await first.agent.post("/api/notifications/push-token").send({ token, platform: "android" });
    await second.agent.post("/api/notifications/push-token").send({ token, platform: "android" });
  ...
  test("requires authentication", async () => {
    ...
    const res = await request(app).post("/api/notifications/push-token").send({ token: VALID_TOKEN, platform: "ios" });
...
describe("unregistering a push token", () => {
  test("removes the caller's own token", async () => {
    ...
    await user.agent.post("/api/notifications/push-token").send({ token, platform: "ios" });

    const del = await user.agent.delete("/api/notifications/push-token").send({ token });
  ...
  test("cannot remove another user's token", async () => {
    ...
    await owner.agent.post("/api/notifications/push-token").send({ token, platform: "ios" });

    await attacker.agent.delete("/api/notifications/push-token").send({ token });
...
describe("notify() sends push to every registered device for that user", () => {
  test("a notification triggers an Expo push send with the right payload", async () => {
    ...
    await user.agent.post("/api/notifications/push-token").send({ token: VALID_TOKEN, platform: "ios" });
    ...
    // notify() is fire-and-forget - give its internal promise a tick to run.
