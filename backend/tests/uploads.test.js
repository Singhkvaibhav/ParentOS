...
describe("image uploads", () => {
  test("requires auth", async () => {
    const res = await (await csrfAgent(app)).post("/api/v1/uploads").send({ imageDataUrl: TINY_PNG });
    expect(res.status).toBe(401);
  });

  test("requires imageDataUrl", async () => {
    const { agent } = await createVerifiedUser(app, { email: "uploadmissing@example.com" });
    const res = await agent.post("/api/v1/uploads").send({});
  ...
  test("rejects data that isn't actually a valid image", async () => {
    ...
    const res = await agent.post("/api/v1/uploads").send({ imageDataUrl: fakeImage });
    expect(res.status).toBe(400);
  ...
  test("accepts a real image, resizes/re-encodes it, and the file is actually retrievable", async () => {
    ...
    const res = await agent.post("/api/v1/uploads").send({ imageDataUrl: TINY_PNG });
  ...
  test("a listing can be created with a real uploaded photo_url", async () => {
    ...
    const uploadRes = await agent.post("/api/v1/uploads").send({ imageDataUrl: TINY_PNG });
...
describe("image cleanup", () => {
  test("deleting a listing removes its stored image from disk", async () => {
    ...
    const deleteRes = await agent.delete(`/api/v1/listings/${listingRes.body.listing.id}`);
  ...
  test("replacing a listing's photo removes the old file but keeps the new one", async () => {
    ...
    const firstUpload = await agent.post("/api/v1/uploads").send({ imageDataUrl: TINY_PNG });
    ...
    const secondUpload = await agent.post("/api/v1/uploads").send({ imageDataUrl: TINY_PNG });
    const newPath = localPathFromUrl(secondUpload.body.url);
    createdFiles.push(newPath);

    await agent.patch(`/api/v1/listings/${listingRes.body.listing.id}`).send({ photoUrl: secondUpload.body.url });
  ...
  test("an update that doesn't touch the photo leaves the file alone", async () => {
    ...
    await agent.patch(`/api/v1/listings/${listingRes.body.listing.id}`).send({ title: "Renamed, same photo" });
