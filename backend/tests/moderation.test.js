...
async function createListing(sellerAgent, overrides = {}) {
  return createListingBase(sellerAgent, { category: "toys", title: "Moderation test toy", priceCents: 1000, payoutReady: false, ...overrides });
}
...
describe("reporting", () => {
  test("a signed-in user can report a listing", async () => {
    ...
    const res = await reporter.agent.post("/api/v1/moderation/reports").send({
  ...
  test("reporting requires authentication", async () => {
    const res = await request(app).post("/api/v1/moderation/reports").send({ listingId: 1, reason: "spam" });
  ...
  test("an invalid reason is rejected", async () => {
    ...
    const listing = await createListing(seller.agent);
    const res = await reporter.agent.post("/api/v1/moderation/reports").send({ listingId: listing.id, reason: "because" });
    expect(res.status).toBe(400);
  });

  test("you can't report yourself", async () => {
    const user = await createVerifiedUser(app, { email: "selfreport@example.com" });
    const res = await user.agent.post("/api/v1/moderation/reports").send({ reportedUserId: user.user.id, reason: "spam" });
    expect(res.status).toBe(400);
  });
  ...
  test("duplicate open reports on the same target are rejected", async () => {
    ...
    await reporter.agent.post("/api/v1/moderation/reports").send({ listingId: listing.id, reason: "spam" });
    const second = await reporter.agent.post("/api/v1/moderation/reports").send({ listingId: listing.id, reason: "spam" });
  ...
  test("reporting a nonexistent listing is a clean 404, not a raw constraint error", async () => {
    ...
    const res = await reporter.agent.post("/api/v1/moderation/reports").send({ listingId: 999999, reason: "spam" });
  ...
  test("you can't report a conversation you're not part of", async () => {
    ...
    const listing = await createListing(seller.agent);
    const send = await buyer.agent.post("/api/v1/messages/thread").send({ listingId: listing.id, text: "hi" });

    const res = await stranger.agent.post("/api/v1/moderation/reports").send({
...
describe("blocking", () => {
  test("blocking prevents a buyer from messaging the seller", async () => {
    ...
    await seller.agent.post("/api/v1/moderation/blocks").send({ userId: buyer.user.id });

    const res = await buyer.agent.post("/api/v1/messages/thread").send({ listingId: listing.id, text: "hello?" });
  ...
  test("blocking is symmetric - the blocker also can't message the blocked user", async () => {
    ...
    await buyer.agent.post("/api/v1/moderation/blocks").send({ userId: seller.user.id });
  ...
  test("a block applied mid-conversation stops further replies", async () => {
    ...
    const send = await buyer.agent.post("/api/v1/messages/thread").send({ listingId: listing.id, text: "first" });
    const conversationId = send.body.conversation.id;

    await seller.agent.post("/api/v1/moderation/blocks").send({ userId: buyer.user.id });

    const res = await buyer.agent.post(`/api/v1/messages/conversations/${conversationId}/reply`).send({ text: "again" });
    expect(res.status).toBe(403);
  });
  ...
  test("unblocking restores messaging", async () => {
    ...
    await seller.agent.post("/api/v1/moderation/blocks").send({ userId: buyer.user.id });
    await seller.agent.delete(`/api/v1/moderation/blocks/${buyer.user.id}`);

    const res = await buyer.agent.post("/api/v1/messages/thread").send({ listingId: listing.id, text: "hi again" });
    expect(res.status).toBe(201);
  });

  test("you can't block yourself", async () => {
    const user = await createVerifiedUser(app, { email: "selfblock@example.com" });
    const res = await user.agent.post("/api/v1/moderation/blocks").send({ userId: user.user.id });
    expect(res.status).toBe(400);
  });
  ...
  test("blocks are listable", async () => {
    ...
    await a.agent.post("/api/v1/moderation/blocks").send({ userId: b.user.id });

    const res = await a.agent.get("/api/v1/moderation/blocks");
...
describe("moderator actions", () => {
  test("a non-admin can't see the report queue or take listings down", async () => {
    ...
    expect((await user.agent.get("/api/v1/moderation/reports")).status).toBe(403);
    expect((await user.agent.post("/api/v1/moderation/listings/1/takedown").send({})).status).toBe(403);
  });
  ...
  test("an admin sees open reports", async () => {
    ...
    const listing = await createListing(seller.agent);
    await reporter.agent.post("/api/v1/moderation/reports").send({ listingId: listing.id, reason: "prohibited" });

    const res = await admin.agent.get("/api/v1/moderation/reports");
  ...
  test("a taken-down listing disappears from browsing and can't be bought", async () => {
    ...
    const browse = await request(app).get("/api/v1/listings?q=Unsafe");
    expect(browse.body.listings.some((l) => l.id === listing.id)).toBe(false);

    const buyAttempt = await buyer.agent.post("/api/v1/transactions/checkout").send({ listingId: listing.id });
  ...
  test("a seller can't relist their way out of a takedown", async () => {
    ...
    expect((await seller.agent.post(`/api/v1/listings/${listing.id}/sold`)).status).toBe(403);
    expect((await seller.agent.post(`/api/v1/listings/${listing.id}/reserve`)).status).toBe(403);

    const stillHidden = await request(app).get("/api/v1/listings");
  ...
  test("an admin can restore a listing they took down", async () => {
    ...
    await admin.agent.post(`/api/v1/moderation/listings/${listing.id}/takedown`).send({ reason: "Mistake" });
    await admin.agent.post(`/api/v1/moderation/listings/${listing.id}/restore`);

    const browse = await request(app).get("/api/v1/listings?q=Restorable");
  ...
  test("resolving a report moves it out of the open queue", async () => {
    ...
    const listing = await createListing(seller.agent);
    const report = await reporter.agent.post("/api/v1/moderation/reports").send({ listingId: listing.id, reason: "misleading" });

    const res = await admin.agent.post(`/api/v1/moderation/reports/${report.body.report.id}/resolve`).send({
      status: "dismissed", note: "Description was accurate",
    });
    expect(res.status).toBe(200);

    const open = await admin.agent.get("/api/v1/moderation/reports?status=open");
  ...
  test("a resolved report allows the same reporter to file again if it recurs", async () => {
    ...
    const first = await reporter.agent.post("/api/v1/moderation/reports").send({ listingId: listing.id, reason: "spam" });
    await admin.agent.post(`/api/v1/moderation/reports/${first.body.report.id}/resolve`).send({ status: "dismissed" });
